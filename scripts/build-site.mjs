...
// Commit ready-to-serve pages beside README.md for Pages: main / (root).
// Keep the existing source assets/data in site/; dist/ remains a portable preview.
for (const page of ['index.html', 'guide.html', 'resources.html', '404.html']) {
  const rendered = await read(`dist/${page}`);
  await writeFile(path.join(root, page), rendered.replaceAll('./assets/', './site/assets/'));
}
await writeFile(path.join(root, '.nojekyll'), '');
console.log('Updated repository-root pages for GitHub Pages branch publishing.');
